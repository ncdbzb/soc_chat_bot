...
from config.config import SMTP_USER, SERVER_DOMAIN, admin_dict
...
def get_accepted_request_email_template(name: str, user_email: str, token: str) -> EmailMessage:
    ...
    email.set_content(
        ...
        f'<p>https://{SERVER_DOMAIN}/logIn?token={token}</p>'
        '</div>',
        subtype='html'
    )
    return email
...
def get_forgot_email_template(name: str, user_email: str, token: str) -> EmailMessage:
    ...
    email.set_content(
        ...
        f'<p>https://{SERVER_DOMAIN}/reset_password?token={token}</p>'
        '</div>',
        subtype='html'
    )
    return email
